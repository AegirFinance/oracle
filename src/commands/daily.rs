...
impl Command {
    ...
    pub async fn run(&self) -> anyhow::Result<()> {
        ...
        let agent = create_agent(&self.ic_url, &self.get_auth(handle)?).await?;

        let deposits_canister_id = Principal::from_text(&self.deposits)?;
        ...
        let governance_canister_id = Principal::from_text(&self.governance)?;
